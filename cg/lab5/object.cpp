...
QVector<CustomPolygon> Object::getObject(){
    return obj;
}
...
QVector3D Object::getCenter(){
    ...
    for (int i = 0; i < obj.size(); i++){
        ...
        y1 = 0;
        z1 = 0;
        n1 = obj[i].getPolygon().size();//.at(i).size();
        for (int j = 0; j < obj[i].getPolygon().size(); j++){
            x1 += obj[i].getPolygon().at(j).x();
            y1 += obj[i].getPolygon().at(j).y();
            z1 += obj[i].getPolygon().at(j).z();
        }
        x0 += x1/n1;
        y0 += y1/n1;
        z0 += z1/n1;
...
QVector3D Object::getSize(){
    float min_x = obj[0].getPolygon().at(0).x(),
          max_x = min_x,
          min_y = obj[0].getPolygon().at(0).y(),
          max_y = min_y,
          min_z = obj[0].getPolygon().at(0).z(),
          max_z = min_z;
    for (int i = 0; i < obj.size(); i++){
        for (int j = 0; j < obj[i].getPolygon().size(); j++){
            if (obj[i].getPolygon().at(j).x() < min_x) min_x = obj[i].getPolygon().at(j).x();
            if (obj[i].getPolygon().at(j).x() > max_x) max_x = obj[i].getPolygon().at(j).x();
            if (obj[i].getPolygon().at(j).y() < min_y) min_y = obj[i].getPolygon().at(j).y();
            if (obj[i].getPolygon().at(j).y() > max_y) max_y = obj[i].getPolygon().at(j).y();
            if (obj[i].getPolygon().at(j).z() < min_z) min_z = obj[i].getPolygon().at(j).z();
            if (obj[i].getPolygon().at(j).z() > max_z) max_z = obj[i].getPolygon().at(j).z();
        }
    }
    return QVector3D(max_x - min_x, max_y - min_y, max_z - min_z);
}

void Object::setPolygon(CustomPolygon p, int pos){
    obj[pos] = p;
}
...
void Object::setPolygonColor(int i, QColor color){
