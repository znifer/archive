...
void sortProb(symb* a, int n) {
	...
	while (pos != n) {
		...
		minPos = pos;
		for (int i = pos; i < n; i++) {
			if (a[i].prob < min || a[i].prob == min && a[i].s > a[minPos].s) { ///CHANGED!!!
				min = a[i].prob;
				minPos = i;
			}
		}
...
symb* readAlphabet(int n, istream& in) {
	symb* a = new symb[n];
...
int main() {
	...
	ifstream in;
	char md;
	cout << "Ввести с файла? (0 - нет, 1 - да): ";
	cin >> md;
	symb* a;
	if (md == '1') {
		in.open("inp.txt");
	...
	string inp;
	cout << "Введите сообщение, которое нужно закодировать: " ;
	cin >> inp;
	cout  << inp.length() << " - длина введенного сообщения" << endl;
	cout << "При кодировании использовать коды Хаффмана(1) или Шеннона-Фано(0)?: ";
	bool isH;
	cin >> isH;

	cout << "Использовать поблочное кодирование? (0 - нет, 1 - да): ";
	int mode;
	cin >> mode; 

	if (mode) {
		int blockSize;
		cout << "Введите длину блоков: ";
		cin >> blockSize;
		if (blockSize > 4) {
		...
		cout << resultStr << endl;
		cout << "Вывести алфавит? (0 - нет, 1 - да) ";
		char c;
		cin >> c;
		if (c == '1'){
			printAlphabet(res, resn);
