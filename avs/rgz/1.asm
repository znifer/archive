...
WndProc proc hWnd:HWND, uMsg:UINT, wParam:WPARAM, lParam:LPARAM
		...
		.elseif uMsg==WM_CREATE ;осылается тогда, когда программа запрашивает, вызовом какой функции CreateWindowEx ,должно быть создано окно
			...
			.IF(eax == 1)
				...
				fadd normX2
				fsqrt
				; st(0) содержит модуль вектора
				fstp len

				fld normX
				fdiv len
				fst normX ; normX - нормализован

				fld normY
				fdiv len
				fst normY ; normY - нормализован

				fild ballDX
				fmul normX
				fild ballDY
				fmul normY
				; Стек - ballDY * normY, ballDX * normX
				faddp st(1), st(0)
				
				mov temp, 2
				fimul temp

				fstp scalar ; Получено скалярное произведение (умноженное на 2)
